          ...
          <transition name="slide-fade">
            <div
              ...
              <el-row>
                <el-col :span="5">
                  政治面貌:
                  <el-select v-model="emp1.politicId" style="width: 130px" size="mini" placeholder="政治面貌">
                    <el-option
                ...
                <el-col :span="4">
                  民族:
                  <el-select v-model="emp1.nationId" style="width: 130px" size="mini" placeholder="请选择民族">
                    <el-option
                ...
                <el-col :span="4">
                  职位:
                  <el-select v-model="emp1.posId" style="width: 130px" size="mini" placeholder="请选择职位">
                    <el-option
                ...
                <el-col :span="4">
                  职称:
                  <el-select v-model="emp1.jobLevelId" style="width: 130px" size="mini" placeholder="请选择职称">
                    <el-option
                ...
                <el-col :span="7">
                  聘用形式:
                  <el-radio-group v-model="emp1.engageForm">
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <el-button @click="toEmpAdv(scope.row)" style="padding: 3px 4px 3px 4px;margin: 2px" type="primary"
                           size="mini">合同
                </el-button>
                <el-button @click="toResume(scope.row)" style="padding: 3px 4px 3px 4px;margin: 2px" type="primary"
                           size="mini">简历
                </el-button>
...
<script>
  export default {
    data() {
      return {
        fileList2: [{name: 'food.jpeg', url: 'https://fuss10.elemecdn.com/3/63/4e7f3a15429bfda99bce42a18cdd1jpeg.jpeg?imageMogr2/thumbnail/360x360/format/webp/quality/100'}, {name: 'food2.jpeg', url: '/img/test.jpg'}],
  ...
  uploadSuccess (response, file, fileList) {
    if (response) {
      this.$message({type: response.status, message:"上传成功"});
    }
  },
  // 上传错误
  uploadError (response, file, fileList) {
    if (response) {
      this.$message({type: response.status, message: "上传失败请重试"});
    }
    },
  //上传图片前堆文件得大小判断
  beforeAvatarUploadImg (file) {
    const extension5=file.name.split(".")[1]==="jpg"
    const extension6=file.name.split(".")[1]==="png"
    const isLt2M = file.size / 1024 / 1024 < 5
    if (!extension5&&!extension6) {

    }
    if (!isLt2M) {
      // this.$alert('上传模板大小不能超过 5MB!', '失败', {
      //   confirmButtonText: '确定'
      // }
      // console.log('上传模板大小不能超过 5MB!')
    }
    return extension5 || extensions6&& isLt2M
  },
  // 上传word前对文件的大小的判断
  ...
  //主题业务逻辑
      toResume(row){
         this.$router.push({ path: '/new/resume', query: { workId:row.workID }})
      },
      toEmpAdv(row){
        this.$router.push({ path: '/new/imfor', query: { workId:row.workID }});
      },
...
//        document.body.appendChild(iframe);
      ...
      loadEmps(){
        var _this = this;
        this.tableLoading = true;
        this.getRequest("/employee/basic/emp?page=" + this.currentPage + "&size=10&keywords=" + this.keywords + "&politicId=" + this.emp1.politicId + "&nationId=" + this.emp1.nationId + "&posId=" + this.emp1.posId + "&jobLevelId=" + this.emp1.jobLevelId + "&engageForm=" + this.emp1.engageForm + "&departmentId=" + this.emp1.departmentId + "&beginDateScope=" + this.beginDateScope).then(resp=> {
